...
## ⚠️ START HERE — 2026-06-16（v0.5.4 ＝腰トラッカー全廃・腿2個から hip を推定＝**実機検証済み GO・push 済み**）

**いまの状態**: `dist/manekko-0.5.4.zip` ＝**実機検証済み・GO・commit/push 済み**。**腰トラッカーを外し**、HTC デモ
のように腿2個から hip（root）を**推定**する。0.5.3 zip はフォールバックとして残置。これで**トラッカーが1個余る**
→ 買い足して手首へ → 将来フィンガートラッカー自作（フレキシブルストラップ抵抗10本＋電子工作）でハーメルの指
キャプチャを不要化、というロードマップの第一歩（GO/NOGO テスト＝GO）。

**何を変えたか（v0.5.4、`src/live.py` のみ・局所追加）**:
- `SYNTH_HIP_FROM_THIGHS=True` ＋ `LiveDriver._synth_hip()`。**hip トラッカーが居なければ**腿2個から合成し、
  `calibrate`／`step` の両方で snapshot に hip を注入：**位置**=`(knee_l+knee_r)/2`（骨盤中心への定数差は A-pose
  キャリブが吸収＝足10cmと同じ）、**向き**=腿ライン(左右軸)＋hip→head(上軸)で骨盤フレーム構築（無いと root yaw
  が無拘束で回れない＝交絡。だから yaw も合成）。
- `TRACKER_TO_BONE` の hip は**残置**＝solver の hip タスク・rest 値をそのまま流用。`SERIAL_TO_ROLE` 無変更＝
  **実 hip トラッカーが居れば自動でそちら優先（フォールバック）**。戻すなら `SYNTH_HIP_FROM_THIGHS=False`。
- **既知の弱点（設計上）**: 中点推定なので片足上げ・深い踏み込み・スクワット等の**非対称な脚姿勢で骨盤が動かした
  脚側へ寄ってブレる**。実機 GO＝立位・通常動作では実用。腿トラッカーが股関節に近い（高い取付）ほどブレが小さい。

**取付位置の確認（オーナー報告・2026-06-16）＆設計上の答え**: 膝＝旧「膝上10cm」→現「**腰から15cm下**」、肘＝
写真同様「上腕骨の**真ん中**」へ移動。**反映は不要**：膝・肘は **orientation-only**（向きのみ使用）で、骨に沿った
取付位置は向き出力に無関係（骨に沿ってどこでも同じ向きを報告）。`BONE_OFFSET_M`（法線オフセット）も向きには
効かない。**むしろ膝が股関節に近い＝高い取付は hip 推定（中点）のブレを減らす方向で好都合**（HTC が上に付ける
理由）。＝取付位置を上げたのは合成 hip にとってプラス。定数いじり不要。

**次にやること（計画確定・2026-06-16・ハード手配中）＝ハンドコントローラ全廃へ**:
オーナーが注文：①**VIVE トラッカーを買い足し**（前腕用。腰外しで余った1個＋新1個＝**2個、両前腕に装着**）②**ESP32×2**で
指＋手首の曲げをフレックスセンサ取得（片手7枚＝指5＋手首2軸）③**スイッチ多め**（キャリブ用＋MENU代用）。

**重要＝コントローラが担う3役と置換先**（②③だけでは①が埋まらない点に注意）:
1. トラックパッド→手首2軸 ⇒ **ESP32 フレックス**で置換。
2. MENUボタン→録画/キャリブ操作 ⇒ **物理スイッチ**で置換。
3. **コントローラ世界向き→前腕 world-FK**（`rig.FK_ORIENT_BONE`: palm_l/r→Forearm、`live._apply_forearm_fk`）
   ⇒ **これはフレックスでは取れない**（6DOF 姿勢であって曲げ量ではない）。**前腕に付ける VIVE トラッカー2個**で
   引き継ぐ。だから「コントローラ休止」が成立するのは前腕トラッカーが③を肩代わりしたとき**だけ**。

**取付＝前腕（手の甲/手首でなく肘下の前腕）**：手の甲だと「前腕向き＋手首曲げ」が混ざり ESP32 の手首曲げと
二重計上になる。前腕に付ければ前腕向きだけ取れ、ESP32 が手首曲げを上乗せ＝分離できる（オーナー合意）。

**ESP32 取り込み設計（実装時の指針）**:
- 伝送＝**Wi-Fi UDP、PC側1ソケット**で2台同時受信（UDP はコネクションレス＝両機が同 host:port に送れる）。
  各パケット先頭に**手ID**を入れて demux。中身＝手ID＋指5＋手首2＋**ボタンビット（スイッチ）**を1パケットに集約
  ＝コントローラ（パッド＋MENU）を丸ごと置換、別受信デバイス不要。
- モーダル tick（main・24fps）で**non-blocking に drain（最新だけ残す＝latest-wins）**。or 小さな受信スレッド
  （bpy 非タッチなら可、`TrackerReader` と同型）。
- **キャリブは2層**：ESP32側＝センサ素の0点/レンジ（装着バラつき吸収）。manekko側＝正規化値→ボーンの軸/符号/
  可動域マッピング（手首は既存 `apply.WRIST_*`、指は v0.5.0 で消した焼き込みを**復活**）。
- **コード変更点（着手時）**: `rig.FK_ORIENT_BONE`/`SERIAL_TO_ROLE`＝前腕向き源を palm→前腕トラッカー役割へ、
  `apply._apply_wrist` の入力源をパッド→ESP32(flex,dev)、指焼き込み再追加、`ops` の MENU エッジ検出→スイッチ
  ビット、新規 UDP 取り込みモジュール。フェイス=niramekko、指の現状キャプチャ=manecam ＝本リポ対象外。

---

## ⚠️ START HERE — 2026-06-10（v0.5.3 ＝指の駆動を全廃＋手首をトラックパッドで2軸・**実機検証済み・完成・旧**）
...
**何を変えたか（速度最適化「案#1」のみ）**: `apply.apply_pose` が**毎フレーム**呼んでいた MuJoCo の名前引き＋
モデル配列読み（`mj_name2id`／`body_jntnum`／`body_jntadr`／`jnt_type`／`jnt_qposadr`／`jnt_axis`）を**初回1回だけ**に。
新ヘルパ `apply._apply_body_table(rm, model)` が body ごとの `(bone名, jtype, qpos_adr, hinge軸)` を前計算し、
`rm._apply_table` にキャッシュ（rm と model は LiveDriver.__init__ で同時生成・独立に作り直さない＝陳腐化しない）。
weld は jtype=None で表現。**ループ本体のロジック・出力は無変更**（リターゲット原理は不変）。`_apply_forearm_fk`
は既に `_build_fk_tables` で同種キャッシュ済み＝今回対象外。

**速度最適化の全体方針（オーナー合意・2026-06-10）**: ❌#0 プロファイル計測は**やらない**（「実際のベストがベスト」）。
✅今回 #1（名前引きキャッシュ）。次は **0.4.2 が壊れていないか実機確認 → その次を決める**。計画中だが今は未実施＝
**#6 solve のワーカースレッド化**（mink solve は bpy 非タッチなので別スレッド可、apply のみ main。1F遅延とライフ
サイクル管理が増える＝後回し）。**#3（毎フレームの rotation_mode 再設定・指スキップ・view_layer.update 粒度）は
別件の都合で保留＝オーナーが次に指示予定**。コメント削除等の「見た目クリーンアップ」は速度に無関係（CPython は
コメントをトークナイズ時に破棄・docstring も定義時1回）＝やらない、で合意。

**次にやること**: ①0.4.2 を実機で「0.4.1 と同じ動き・壊れていない」を確認 ②壊れていなければ commit/push ③その後
#3 をオーナー指示で着手、または #6 を検討。

---

## ⚠️ START HERE — 2026-06-07（v0.4.1 ＝膝（下半身）追加・**実機検証済み**・旧）

**いまの状態**: `dist/manekko-0.4.1.zip` ＝**実機検証済み・commit/push 済み**。全身（上半身＋膝）が動く。
0.3.4 は 0.3系最終版（フォールバック）。**今朝はここで一区切り。たぶん夕方から継続。**

**膝（v0.4.1、肘と同型）**: 膝トラッカー（腿、膝関節の5cm上）→ **Thigh の向き**を orientation-only/cost=5.0 で
厳密固定（`rig.KNEE_ORIENT_BONE`、`solver.orientation_only_roles` に knee_l/r）。空いた手首トラッカー2個を
膝に再割当（`SERIAL_TO_ROLE`: `LHR-8CBC92B3`→knee_l, `LHR-4CEBC3D1`→knee_r。**左右逆なら2行入替**）。足は
既存どおり Foot 位置＋向きIK（腿固定の下で Calf 膝ヒンジを mink が解く＝すねにトラッカー無いため位置IK必須）。
足トラッカーは「かかとから10cm手前」だが定数オフセットは A-pose キャリブが吸収。

**次にやること（未着手）**:
- **トラックパッドで手首2軸**。先に「必要動作の洗い出し（例: Vサイン等のジェスチャ）」＝設計してから実装、と
  オーナー方針。Hand は現在 weld（手首剛体）→ そのとき駆動関節化。
- 余力で foot トラッカー10cmオフセットの動的補正（今は A-pose 吸収で足回転時に多少ドリフトしうる＝下半身は
  重要度低めで許容）。

---

## START HERE — 2026-06-07（v0.4.0 ＝コントローラで前腕ワールドFK・実機検証済み・旧）

**いまの状態**: `dist/manekko-0.4.0.zip` ＝実機検証済み。腕が肘トラッカー＋コントローラで動く。
